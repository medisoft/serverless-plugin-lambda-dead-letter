...
class Plugin {
  constructor (serverless, options) {
    ...
    this.hooks = {
      'deploy:deploy': () => BbPromise.bind(this).then(this.setLambdaDeadLetterConfig),

      'deploy:compileEvents': () =>
        BbPromise.bind(this).then(this.compileFunctionDeadLetterResources),
  ...
  resolveTargetArn (functionName, deadLetter, resolveStackResources) {
    ...
    if (targetDefCount === 0) {
      throw new Error(
        `Function: ${functionName}.deadLetter is missing one of the following properties: [sqs, sns, targetArn]`
      )
    }

    if (targetDefCount > 1) {
      throw new Error(
        `Function: ${functionName}.deadLetter can only have one of the following properties: [sqs, sns, targetArn]`
      )
    }

    if (deadLetter.sqs !== undefined) {
      return this.resolveTargetArnFromObject(
        functionName,
        {
    ...
    throw new Error(
      `Function property ${functionName}.deadLetter.targetArn is an unexpected type.  This must be an object or string.`
    )
  }
  ...
  resolveTargetArnFromString (functionName, targetArn) {
    ...
    if (!rg.test(targetArn)) {
      throw new Error(
        `Function property ${functionName}.deadLetter.targetArn = '${targetArn}'.  This is not a valid sns or sqs arn. `
      )
    }
  ...
  resolveTargetArnFromObject (functionName, targetArn, resolveStackResources) {
    if (!targetArn.GetResourceArn) {
      throw new Error(
        `Function property ${functionName}.deadLetter.targetArn object is missing GetResourceArn property.`
      )
    }
    ...
    return this.provider
      ...
      .then(response => {
        ...
        switch (resType) {
          ...
          default:
            throw new Error(
              `Function property ${functionName}.deadLetter.targetArn.GetResourceArn ` +
  ...
  buildDeadLetterUpdateParams (functionName, resolveStackResources) {
    ...
    return BbPromise.bind(this)
      ...
      .then(() =>
        this.resolveTargetArn(functionName, functionObj.deadLetter, resolveStackResources)
      )
  ...
  setLambdaDeadLetterConfig () {
    return BbPromise.mapSeries(this.serverless.service.getAllFunctions(), functionName =>
      this.buildDeadLetterUpdateParams(functionName, this.deploy).then(deadLetterUpdateParams => {
        ...
        if (this.deploy) {
          logPrefix = '(updated)'
          updateStep = this.provider.request(
            'Lambda',
            'updateFunctionConfiguration',
            deadLetterUpdateParams,
            this.options.stage,
            this.options.region
          )
        } else {
        ...
        return updateStep.then(() => {
          ...
          this.serverless.cli.log(
            `${logPrefix} Function '${functionName}' ` +
  ...
  compileFunctionDeadLetterResource (functionName) {
    ...
    if (functionObj.deadLetter.sqs !== undefined) {
      return this.compileFunctionDeadLetterQueue(functionName, functionObj.deadLetter.sqs)
    }

    if (functionObj.deadLetter.sns !== undefined) {
      return this.compileFunctionDeadLetterTopic(functionName, functionObj.deadLetter.sns)
    }
  ...
  compileFunctionDeadLetterQueue (functionName, queueConfig) {
    ...
    if (typeof queueConfig === 'string' || queueConfig === null) {
      ...
      throw new TypeError(
        `Function property ${functionName}.deadLetter.sqs is an unexpected type.  This must be an object or a string.`
      )
    }
    ...
    const queuePolicyResource = {
      ...
      Properties: {
        ...
        PolicyDocument: {
          Id: {
            'Fn::Join': ['', [{ 'Fn::GetAtt': [queueLogicalId, 'Arn'] }, '/SQSDefaultPolicy']]
          },
          Version: '2012-10-17',
          Statement: [
            {
  ...
  compileFunctionDeadLetterTopic (functionName, topicConfig) {
    if (typeof topicConfig !== 'string' && topicConfig !== null) {
      throw new Error(
        `Function property ${functionName}.deadLetter.sns is an unexpected type.  This must be a or string.`
      )
    }
    ...
    if (topicName.length < 1) {
      throw new Error(
        `Function property ${functionName}.deadLetter.sns must contain one or more characters.`
      )
    }
